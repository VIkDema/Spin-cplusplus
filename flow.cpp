	...
	{	if (!e->n
		...
		case D_STEP:
			...
			if (0) printf("link %d, {%d .. %d} -> %d (ntyp=%d) was %d\n",
				...
				e->n->sl->this_sequence->last->seqno,
				f?f->seqno:-1, f?f->n->ntyp:-1,
				e->n->sl->this_sequence->last->nxt?e->n->sl->this_sequence->last->nxt->seqno:-1);
...
{	int	tok = n->ntyp;
	...
	if (prev_z
	&&  ref_chans
	&&  prev_z->this_sequence->frst->n->ntyp == ELSE)
...
{	SeqList	*s  = n->sl;
	...
	{	Element *x; printf("\t%d,%d,%d :: ",
		z->this_sequence->frst->Seqno,
		z->this_sequence->extent->Seqno,
		z->this_sequence->last->Seqno);
