...
static const char *Pre0[] = {
    "#ifdef SC",
    "	#define _FILE_OFFSET_BITS	64", /* allows file sizes greater than
                                                2Gb */
    "#endif",
    "#include <stdio.h>",
    "#include <signal.h>",
...
static const char *Preamble[] = {
    ...
    "long nr_handoffs = 0;",
    "#endif",
    "uchar	warned = 0, iterative = 0, exclusive = 0, like_java = 0, "
    "every_error = 0;",
...
static const char *Tail[] = {
    ...
    "#endif",
    "void",
    "retrans(int n, int m, int is, short srcln[], uchar reach[], uchar "
    "lpstate[])",
    ...
    "			procname[n]);",
    "		for (i = 1; i < m; i++)",
    "		for (T0 = trans[n][i]; T0; T0 = T0->next)",
    "			crack(n, i, T0, srcln);",
    "		return;",
    "	}",
    "	do {",
    "		for (i = 1, cnt = 0; i < m; i++)",
    "		{	T2 = trans[n][i];",
    "			T1 = T2?T2->next:(Trans *)0;",
    "/* prescan: */		for (T0 = T1; T0; T0 = T0->next)",
    ...
    "	}",
    "	for (i = 1; i < m; i++)",
    "	{	if (trans[n][i] && trans[n][i]->next) /* optimize */",
    "		{	T1 = trans[n][i]->next;",
    "#if 0",
    ...
    "		{	if (p = T0->escp[k])",
    "			for (T1 = trans[n][p]; T1; T1 = T1->next)",
    ...
    "			if (progstate[n][i])",
    "				goto degrade;",
    "			for (T1 = trans[n][i]; T1; T1 = T1->next)",
    ...
    "			if (!(T1->atom&8)) break;",
    "		if (T1)",
    "degrade:	for (T1 = trans[n][i]; T1; T1 = T1->next)",
    ...
    "			}",
    "	#endif",
    "			/* T4 points to first non-escape, T5 to its parent, T0 "
    "to original list */",
    "			if (T4 != T0)		 /* there was at least one "
    "escape */",
    "			{	T3 = T5->next;		 /* start of "
    "non-escapes */",
    "				T5->next = (Trans *) 0;	 /* separate */",
    "				Tx = T0;		 /* start of the "
    "escapes */",
    "				Ty = T5;		 /* its tail */",
    ...
    "reversed */",
    "			}",
    "			/* T0 points to first non-escape, Tx to the list of "
    "escapes, Ty to its tail */",
    "",
    ...
    "					T1 = T5;",
    "			}	}",
    "			/* T3 points to a possible else, which is removed from "
    "the list */",
    ...
    "			{	T2->next = T3;	/* add else */",
    "			} else",
    "			{	if (T3) /* there was an else, but there's no "
    "tail */",
    "				{	if (!T1)	/* and no reversed "
    "list */",
    ...
    "			}	}	}",
    "",
    "			/* add in the escapes, to that they appear at the "
    "front */",
    "			if (Tx && Ty) { Ty->next = T1; T1 = Tx; }",
    "",
    "			trans[n][i] = T1;",
    ...
    "",
    "void",
    "dfs_table(int n, int m, int is, short srcln[], uchar reach[], uchar "
    "lpstate[])",
    "{	Trans *z;\n",
    ...
    "	fflush(stdout);",
    "}",
    "/* spin -a m.pml; cc -o pan pan.c; ./pan -D | dot -Tps > foo.ps; ps2pdf "
    "foo.ps */",
    "void",
    ...
    "",
    "	if (v<0 || v > 255) return;",
    "	for (tmp = ranges; tmp; tmp = tmp->next)",
    "		if (!strcmp(tmp->nm, s))",
    "			goto found;",
