...
static const char *Code2c[] = {
    ...
    "",
    "struct SM_frame     SM_frame;",
    "struct SM_results   SM_results;",
    "struct sh_Allocater sh_Allocater;",
    "",
    ...
    "};",
    "",
    "int	proxy_pid;		/* id of proxy if nonzero -- receive "
    "half */",
    "int	store_proxy_pid;",
    "short	remote_party;",
    ...
    "};",
    "",
    "int		core_id = 0;		/* internal process nr, to "
    "know which q to use */",
    ...
    "static   int	lrfree;		/* used for temporary recording of "
    "slot */",
    ...
    "#ifdef SEP_STATE",
    "	if (0)",
    "	{	cpu_printf(\"nstates %%g nshadow %%g -- memory %%-6.3f "
    "Mb\\n\",",
    ...
    "		}",
    "		if (core_id == 0)",
    "		{	if (((*search_terminated) & 4)	/* uerror in one of "
    "the cpus */",
    ...
    "					(int) worker_pids[i], (dwExitCode == "
    "STILL_ACTIVE));",
    "#else",
    "				sprintf(b, \"kill -%%d %%d\", (int) SIGKILL, "
    "(int) worker_pids[i]);",
    "				ignore = system(b);	/* if this is a proxy: "
    "receive half */",
    "				printf(\"cpu0: %%s\\n\", b);",
    "#endif",
    "		}",
    ...
    "		if (was_interrupted == 0)", /* 2=SIGINT to root to trigger stop
                                             */
    "		{	int ignore;",
    "			sprintf(b, \"kill -%%d %%d\", (int) SIGINT, (int) "
    "worker_pids[0]);",
    ...
    "		if (core_id == 0 && (*gr_readmiss >= 1.0 || *gr_readmiss >= "
    "1.0 || *grcnt != 0))",
    "		printf(\"cpu0: gq [readmiss: %%g, writemiss: %%g cnt "
    ...
    "	#ifndef NGQ",
    "	if (!a_cycles)",
    "	{	printf(\"hint: this search may be faster with a larger "
    "work-queue\\n\");",
    ...
    "z_handoff);",
    "	#else",
    "	{	printf(\"hint: this search may be faster if compiled without "
    "-DNGQ, with -DUSE_DISK, \");",
    ...
    "if N>1\\n\");",
    "	fprintf(fd, \"\\n\");",
    "	fprintf(fd, \"Additional directives supported in multi-core "
    "mode:\\n\\n\");",
    ...
    "changing):\\n\\n\");",
    "	fprintf(fd, \"     To change the nr of states that can be stored in "
    "the global queue\\n\");",
    ...
    "	fprintf(fd, \"\\n\");",
    "	fprintf(fd, \"     Timer settings for termination and crash "
    "detection:\\n\");",
    "	fprintf(fd, \"        -DSHORT_T=N --> timeout for termination "
    "detection trigger (N=%%g)\\n\", (double) SHORT_T);",
    "	fprintf(fd, \"        -DLONG_T=N  --> timeout for giving up on "
    "termination detection (N=%%g)\\n\", (double) LONG_T);",
    "	fprintf(fd, \"        -DONESECOND --> (1<<29) --> timeout waiting for "
    "a free slot -- to check for crash\\n\");",
    ...
    "",
    "	if (nr_cached == 0)",
    "	{	stack_cache = (char *) grab_shared(CACHE_NR * "
    "sizeof(Stack_Tree));",
    "		nr_cached = CACHE_NR;",
    "	}",
    ...
    "		} else /* iamin[which] <= 0 */",
    "		{	if (!readtrail)",
    "			{	printf(\"cpu%%d: Invalid Unlock iamin[%%d] = "
    "%%d\\n\",",
    "					core_id, which, iamin[which]);",
    "				fflush(stdout);",
    "			}",
    ...
    "			}",
    "			shmid[m] = shmget(key[m], (size_t) qsize, "
    ...
    "		}",
    "",
    "		shared_mem[m] = (char *) shmat(shmid[m], (void *) 0, "
    "0);	/* attach */",
    ...
    "",
    "static uchar *",
    "prep_shmid_S(size_t n)		/* either sets SS or H_tab, "
    "linux/cygwin */",
    "{	char	*rval;",
    "#ifndef SEP_STATE",
    "	key_t	key;",
    ...
    "{	char	*rval;",
    "	key_t	key;",
    "	static int cnt = 3;		/* start larger than earlier ftok "
    "calls */",
    "",
    "	shm_prep_result = NOT_AGAIN;	/* default */",
    "	if (verbose && core_id == 0)",
    "	{	printf(\"cpu0: step 2+: pre-allocate memory arena %%d of "
    "%%6.2g Mb\\n\",",
    ...
    "	if (shmid_M == -1)",
    "	{	if (verbose)",
    "		{	printf(\"error: failed to get pool of shared memory "
    "%%d of %%.0f Mb\\n\",",
    "				cnt-3, ((double)n)/(1048576.));",
    "			perror(\"state mem\");",
    "			printf(\"pan: check './pan --' for usage "
    "details\\n\");",
    "		}",
    ...
    "",
    "	if ((char *) rval == (char *) -1)",
    "	{	printf(\"cpu%%d error: failed to attach pool of shared memory "
    "%%d of %%.0f Mb\\n\",",
    ...
    " #else",
    "	if (verbose)",
    "	{	printf(\"cpu0: steps 0,1: -DMEMLIM=%%d Mb - (hashtable %%g Mb "
    "+ workqueues %%g Mb)\\n\",",
    "		MEMLIM, ((double)n/(1048576.)), (((double) NCORE * LWQ_SIZE) + "
    "GWQ_SIZE) /(1048576.) );",
    "	}",
    " #endif",
    "	get_mem = NCORE * sizeof(double) + (1 + CS_NR) * sizeof(void *) + "
    "4*sizeof(void *) + 2*sizeof(double);",
    ...
    "",
    "	if (core_id == 0 && verbose)",
    "	{	printf(\"cpu0: step 0: -DMEMLIM=%%d Mb minus hashtable+workqs "
    "(%%g + %%g Mb) leaves %%g Mb\\n\",",
    ...
    "",
    "	if (core_id == 0 && verbose)",
    "	{	printf(\"cpu0: step 2: pre-allocate shared state memory %%g "
    "Mb\\n\",",
    "			need_mem/(1048576.));",
    "	}",
    "#ifdef SEP_HEAP",
    ...
    "	{	printf(\"warning: reducing SEG_SIZE of %%g MB to 512MB "
    "(exceeds max for Cygwin)\\n\",",
    "			SEG_SIZE/(1024.*1024.));",
    ...
    "",
    "	if (need_mem > 1024.)",
    "	{	printf(\"cpu0: could allocate only %%g Mb of shared memory "
    "(wanted %%g more)\\n\",",
    ...
    "		{	if (verbose)",
    "			{	perror(\"shmdt detaching from shared bitstate "
    "arena\");",
    "		}	}",
    "	#else",
    "		if (core_id == 0)",
    ...
    "must attach */",
    "	{	rm_shared_segments();	/* mark all shared segments for "
    "removal on exit */",
    ...
    "		{	if (verbose)",
    "			{	cpu_printf(\"timeout waiting for free slot "
    "q%%d\\n\", n);",
    "			}",
    "			cnt_start = free_wait;",
    "			if (someone_crashed(1))",
    ...
    "*/",
    "				*gr_readmiss++;",
    "				goto out; /* don't wait: release lock and "
    "return */",
    "			}",
    "			lrfree = *grfree;	/* Get_Free_Frame use lrfree "
    "in this mode */",
    ...
    "",
    "		if (f != &cur_Root)",
    "		{	memcpy((uchar *) &cur_Root, (uchar *) f, "
    "sizeof(SM_frame));",
    "		}",
    "",
    "		if (((now._a_t) & 1) == 1)	/* i.e., when starting nested "
    "DFS */",
    ...
    "			}",
    "#ifdef TESTING",
    "	/* to profile with cc -pg and gprof pan.exe -- set handoff depth "
    "beyond maxdepth */",
    "			exit(0);",
    "#endif",
    "			remember = *grfree;",
    ...
    "				&&  *grfree + 1 == remember)	/* no action "
    "on global queue meanwhile */",
    ...
    "",
    "				if (*grfree + 1 == remember)	/* no action "
    "on global queue */",
    ...
    "		if (f->m_boq == QUERY_F)",
    "		{	if (verbose)",
    "			{	cpu_printf(\"got QUERY_F on q%%d from slot "
    "%%d\\n\", q, prfull[q]-1);",
    "			}",
    "			f->m_vsize = 0;	/* release slot */",
    "",
    ...
    "memcnt/(1048576.));",
    "			retrieve_info((SM_results *) f); /* collect and "
    "combine stats */",
    "			if (verbose)",
    ...
    "			if (core_id != 0)",
    "			{	f = Get_Free_Frame(target_q); /* new outgoing "
    "slot */",
    ...
    "#endif",
    "	if (now._nr_pr > 0)",
    "	{ memcpy((uchar *) f->m_p_offset, (uchar *) proc_offset, now._nr_pr * "
    "sizeof(OFFT));",
    "	  memcpy((uchar *) f->m_p_skip,   (uchar *) proc_skip,   now._nr_pr * "
    "sizeof(uchar));",
    "	}",
    "	if (now._nr_qs > 0)",
    "	{ memcpy((uchar *) f->m_q_offset, (uchar *) q_offset, now._nr_qs * "
    "sizeof(OFFT));",
    ...
    "#endif",
    "	if (now._nr_pr > 0)",
    "	{	memcpy((uchar *)f.m_p_offset, (uchar *)proc_offset, "
    "now._nr_pr*sizeof(OFFT));",
    "		memcpy((uchar *)f.m_p_skip,   (uchar *)proc_skip,   "
    "now._nr_pr*sizeof(uchar));",
    "	}",
    "	if (now._nr_qs > 0)",
    "	{	memcpy((uchar *) f.m_q_offset, (uchar *) q_offset, "
    "now._nr_qs*sizeof(OFFT));",
    ...
    "",
    "	if (core_id == 0 && verbose)",
    "	{	printf(\"cpu0: step 3: allocate shared work-queues %%g "
    "Mb\\n\",",
    "			((double) NCORE * LWQ_SIZE + GWQ_SIZE) / (1048576.));",
    "	}",
    ...
    "		{ fprintf(stderr, \"cpu%%d: cannot attach shared q%%d (%%d "
    "Mb)\\n\",",
    ...
    "	}",
    "",
    "	rval = (char *) MapViewOfFile(shmid_S, FILE_MAP_ALL_ACCESS, 0, 0, "
    "0);	/* attach */",
    "	if ((char *) rval == NULL)",
    "	{ fprintf(stderr, \"cpu%%d: cannot attach shared bitstate or "
    "hashtable\\n\", core_id);",
    ...
    "",
    "	if (verbose && core_id == 0)",
    "	{	printf(\"cpu0: step 2+: pre-allocate memory arena %%d of %%g "
    "Mb\\n\",",
    "			cnt-3, (double) n / (1048576.));",
    "	}",
    "	#ifdef MEMLIM",
    "	if (memcnt + (double) n > memlim)",
    "	{	printf(\"cpu%%d: error: M %%.0f + %%.0f exceeds memory limit "
    "of %%.0f Kb\\n\",",
    ...
    "		return NULL;",
    "	}",
    "	rval = (char *) MapViewOfFile(shmid_M, FILE_MAP_ALL_ACCESS, 0, 0, "
    "0);	/* attach */",
    "",
    "	if (rval == NULL)",
    "	{ printf(\"cpu%%d: failed to attach pool of shared memory nr %%d of "
    "size %%d\\n\",",
    "		core_id, cnt-3, n);",
    "	  return NULL;",
    "	}",
    ...
    "",
    "	if (core_id == 0 && verbose)",
    "	{	printf(\"cpu0: step 4: creating additional workers (proxy "
    "%%d)\\n\",",
    "			proxy_pid);",
    "	}",
    "#if 0",
    ...
    "cmdline);",
    "",
    "			is_parent = CreateProcess(0, cmdline, 0, 0, FALSE, 0, "
    "0, 0, &si, &pi);",
    "			if (is_parent == 0)",
    "			{	Uerror(\"fork failed\");",
    "			}",
