...
static const char *Code2a[] = {
    ...
    "		#else",
    "		ncomps = (ulong *) sh_pre_malloc((ulong)((256+2) * "
    "sizeof(ulong)));",
    "		#endif",
    "	#endif",
    "#endif",
    "	iniglobals(258); /* arg outside range of pids */",
    "#if defined(VERI) && !defined(NOREDUCE) && !defined(NP) && !defined(BFS) "
    "&& !defined(HAS_LTL)",
    "	if (!state_tables",
    "	#ifdef HAS_CODE",
    "	&& !readtrail",
    ...
    "		#define NR_QS		(NCORE)",
    "		#define CS_NR		(CS_N+1)	/* 2^N + 1, nr "
    "critical sections */",
    "		#define GQ_RD		GLOBAL_LOCK", /* not really used in this
                                                         mode */
    "		#define GQ_WR		GLOBAL_LOCK", /* but just in case... */
    "		#define CS_ID		(1 + (int) (j1_spin & (CS_N-1))) /* "
    "mask: 2^N - 1, zero reserved */",
    "		#define QLOCK(n)	(1+n)", /* overlaps first n zones of
                                                   hashtable */
    "	#else",
    ...
    "		#define QLOCK(n)	(3+n)", /* overlaps first n zones of
                                                   hashtable */
    "	#endif",
    "",
    "	#ifndef SEP_STATE",
    ...
    "	#else",
    "		#ifdef NGQ",
    "			#define enter_critical(w)	{ if (w < 1+NCORE) "
    "e_critical(w); }",
    "			#define leave_critical(w)	{ if (w < 1+NCORE) "
    "x_critical(w); }",
    "		#else",
    "			#define enter_critical(w)	{ if (w < 3+NCORE) "
    "e_critical(w); }",
    "			#define leave_critical(w)	{ if (w < 3+NCORE) "
    "x_critical(w); }",
    "		#endif",
    "	#endif",
    "",
    ...
    "		II = i;",
    "		z = (P0 *)pptr(II);",
    "		for (t = trans[z->_t][z->_p]; t; t = t->next)",
    "			if (t->t_id == (T_ID) t_id)",
    "				break;",
    "		if (!t)",
    ...
    "	{",
    "#ifdef PUTPID",
    "		sprintf(fnm, \"%%s_%%s_%%d.%%s\", MyFile, progname, getpid(), "
    "tprefix);",
    "#else",
    "		sprintf(fnm, \"%%s.%%s\", MyFile, tprefix);",
    "#endif",
    ...
    "	#else",
    "		#if VECTORSZ>32000",
    "		&&  (memcmp((char *) proc_offset, (char *) h->po, now._nr_pr * "
    "sizeof(int)) == 0)",
    ...
    "			memcpy((char *) h->ps, (char *) proc_skip,   "
    "now._nr_pr * sizeof(uchar));",
    "		#if VECTORSZ>32000",
    "			h->po = (char *) emalloc(now._nr_pr * sizeof(int));",
    "			memcpy((char *) h->po, (char *) proc_offset, "
    "now._nr_pr * sizeof(int));",
    "		#else",
    "			h->po = (char *) emalloc(now._nr_pr * sizeof(short));",
    "			memcpy((char *) h->po, (char *) proc_offset, "
    "now._nr_pr * sizeof(short));",
    "		#endif",
    "		}",
    "		if (now._nr_qs > 0)",
    ...
    "	if (now._nr_pr > 0)",
    "		#if VECTORSZ>32000",
    "	{	memcpy((char *)proc_offset, (char *)t->omask->po, now._nr_pr * "
    "sizeof(int));",
    "		#else",
    "	{	memcpy((char *)proc_offset, (char *)t->omask->po, now._nr_pr * "
    "sizeof(short));",
    "		#endif",
    "		memcpy((char *)proc_skip,   (char *)t->omask->ps, now._nr_pr * "
    "sizeof(uchar));",
    "	}",
    "	if (now._nr_qs > 0)",
    "		#if VECTORSZ>32000",
    ...
    "sizeof(short));",
    "		#endif",
    "		memcpy((uchar *)q_skip,   (uchar *)t->omask->qs, now._nr_qs * "
    "sizeof(uchar));",
    "	}",
    "	#endif",
    "	#ifdef BFS_DISK",
    ...
    "	E_state = 0;",
    "		#endif",
    "	for (t2 = trans[ot][tt]; t2; t2 = t2?t2->next:(Trans *)0)",
    "	{",
    "		#ifdef HAS_UNLESS",
    ...
    "			if (!warned)",
    "			{	warned = 1;",
    "		  		printf(\"error: max search depth too "
    "small\\n\");",
    "			}",
    "			if (bounded)",
    ...
    "				trpt--;",
    "	#ifdef CHECK",
    "				enter_critical(GLOBAL_LOCK);	/* verbose "
    "mode */",
    "		#if NCORE>1",
    "				printf(\"cpu%%d: \", core_id);",
    "		#endif",
    ...
    "		}",
    "	#ifdef DEBUG",
    "		printf(\"%%3ld: no move [II=%%d, tau=%%d, boq=%%d, "
    "_nr_pr=%%d]\\n\",",
    ...
    "			{	continue;		/* root state */",
    "			}",
    "			if ((x->tau&8) || (x->tau&32)) /* break atomic or "
    "preselect at parent */",
...
static const char *Code2d[] = {
    ...
    "	delta_time = ((double) (stop_time - start_time)) / ((double) "
    "sysconf(_SC_CLK_TCK));",
    "#else",
    "	stop_time = clock();",
    "	delta_time = ((double) (stop_time - start_time)) / ((double) "
    ...
    "CLOCKS_PER_SEC);",
    "#else",
    "		delta_time = ((double) (times(&start_tm) - crash_stamp)) / "
    ...
    "CLOCKS_PER_SEC);",
    "#else",
    "	delta_time = ((double) (times(&start_tm) - crash_stamp)) / ((double) "
    "sysconf(_SC_CLK_TCK));",
    "#endif",
    ...
    "	for (t = trans[EVENT_TRACE][now._event]; t; t = t->next)",
    ...
    "	{	_this = pptr(i);",
    "		if (i != pid+BASE",
    "		&&  ((P0 *)_this)->_priority > ((P0 "
    "*)pptr(pid+BASE))->_priority",
    "#ifdef HAS_PROVIDED",
    "		&&  provided(i, (uchar) ((P0 *)_this)->_t, (int) ((P0 "
    "*)_this)->_p, 0)",
    "#endif",
    ...
    "	{",
    "	#ifdef VERBOSE",
    "		printf(\"warning: bad pid %%d, no such process "
    "(set_priority)\\n\", pid);",
    "	#endif",
    "		return 1;",
    "	}",
    ...
    "	#define CONTINUE  { if (reversing&2) { p_reorder(seed); II = oII; } "
    "continue; }",
    "#else",
    ...
    "		#endif",
    "#else",
    "		#ifdef FULLSTACK", /* b_store after onstack_now, to preserve
                                      j1-j4 */
    "		   #if defined(BCS) && defined(STORE_CTX)",
    "		   { int xj;",
    "			now._ctx = 0;",
    ...
    "			for (xj = trpt->sched_limit; xj <= sched_max; xj++)",
    "			{	now._ctx = xj;",
    "				II = b_store((char *)&now, vsize);", /* sets
                                                                        j1-j4 */
    "				if (II != 0) { break; }",
    "			}",
    "			now._ctx = 0;",
    ...
    "		#ifndef NOFAIR",
    "			if (a_cycles && !fairness) /* 5.1.6 -- example by "
    "Hirofumi Watanabe */",
    "		#endif",
    ...
    "				{	int oh = HASH_NR;",
    "					HASH_NR = 0;",
    "					d_hash((uchar *) &now, vsize); /* SHO "
    "- set K1 */",
    "					HASH_NR = oh;",
    "				}",
    "				if (write(svfd, (uchar *) &K1, sizeof(ulong)) "
    "!= sizeof(ulong))",
    "	#else",
    "				if (write(svfd, (uchar *) &now, vprefix) != "
    "vprefix)",
    "	#endif",
    "				{	fprintf(efd, \"writing %%s.svd "
    "failed\\n\", PanSource);",
    "					wrapup();",
    "				}",
    "#endif",
    ...
    "*/",
    "	#ifdef VERBOSE",
    "		printf(\"%%3ld: BCS c_switch phase=%%d pno=%%d [forced "
    "%%d]\\n\",",
    ...
    "	#ifdef VERBOSE",
    "		printf(\"%%3ld: P_RAND skipping %%d [new p_skip=%%d "
    "p_left=%%d]\\n\",",
    ...
    "		{",
    "	#ifdef VERBOSE",
    "		printf(\"%%3ld: BCS NotPre II=%%d bcs=%%d pno=%%d [forced "
    "%%d]\\n\",",
    ...
    "#ifdef T_RAND",
    "		for (ooi = eoi = 0, t = trans[ot][tt]; t; t = t->next, ooi++)",
    ...
    "#else",
    "	cpu_printf(\"%%3ld: proc %%d exec %%d, %%d to %%d, %%s %%s %%s "
    "%%saccepting [tau=%%d]\\n\", ",
    ...
    "depth, II);",
    "			} else",
    "			{	printf(\"%%4d: process %%d modified but gone "
    "(%%p)\\n\",",
    "					depth, II, trpt);",
    "	#endif",
    "			}",
    ...
    "#ifdef VERBOSE",
    "			cpu_printf(\"%%3ld: proc %%d reverses %%d, %%d to "
    "%%d\\n\",",
    "				depth, II, t->forw, tt, t->st);",
    "			cpu_printf(\"\\t%%s "
    "[abit=%%d,adepth=%%ld,tau=%%d,%%d]\\n\", ",
    ...
    "		if (_n == 0 || !(trpt->tau&64))	/* pre-move unexecutable or "
    "led to stackstate */",
    ...
    "	{ double fp = (100. * (double) nstates)/((double) ((ulong) "
    "(1<<ssize)));",
    "	  int fi = 0;",
    ...
    "			while (fp > 2.) { fi++; fp /= 2.; }",
    "			if (fi > 0)",
    "			{	printf(\" (hint: rerun with -w%%d to reduce "
    "runtime)\",",
    "				ssize-fi);",
    "		}	}",
    "		printf(\"\\n\");",
    ...
    "	else",
    "	printf(\"\\nhash factor: %%4g (best if > 100.)\\n\\n\",",
    "		((double)(((ulong)1)<<(ssize-10)) / (double) nstates) * "
    "1024.0);",
    ...
    "		#if !defined(MA) || defined(COLLAPSE)",
    "			#ifdef BFS_PAR",
    "				printf(\"%%9.3f\tshared memory used for hash "
    "table (-w%%d)\\n\",",
    "				((double) bfs_pre_allocated)/1048576., ssize);",
    "			#else",
    "				printf(\"%%9.3f\tmemory used for hash table "
    "(-w%%d)\\n\",",
    "					nr3/1048576., ssize);",
    "				remainder -= nr3;",
    "			#endif",
    ...
    "",
    "void",
    "d_hash(uchar *kb, int nbytes)", /* sets two 64-bit or 32-bit nrs, depending
                                        on WS */
    "{	uint8_t  *bp;",
    "#if WS>4",
    "	uint64_t a = 0, b, c, n;",
    ...
    "",
    "void",
    "s_hash(uchar *cp, int om)", /* uses either d_sfh (1x32bit), or d_hash
                                    (2x64bit) */
    ...
    "#ifndef SAFETY",
    "	#ifdef NP",
    "		case 'a': fprintf(efd, \"warning: -a is disabled by -DNP, "
    "ignored\\n\");",
    "			  break;",
    "	#else",
    ...
    "#endif",
    "		case 'h':",
    "			  if (strncmp(&argv[1][1], \"hash\", strlen(\"hash\")) "
    "== 0)",
    "			  {	do_hashgen = 1;",
    "				break;",
    "			  }",
    ...
    "			  }",
    "			  HASH_NR = "
    "atoi(&argv[1][2])%%(sizeof(HASH_CONST)/sizeof(uint));",
    "			  break;",
    "		case 'I': iterative = 2; every_error = 1; break;",
    "		case 'i':",
    "			  if (strncmp(&argv[1][1], \"i_reverse\", "
    "strlen(\"i_reverse\")) == 0)",
    "			  {	reversing |= 1;",
    "			  } else",
    "			  {	iterative   = 1;",
    ...
    "#else",
    "		case 'l': fprintf(efd, \"error: -l not available (compile with "
    "-DNP)\");",
    "			  usage(efd); break;",
    "#endif",
    "#endif",
    ...
    "#else",
    "		case 'M': case 'G':",
    "			  fprintf(stderr, \"-M and -G affect only "
    "-DBITSTATE\\n\");",
    "			  break;",
    "#endif",
    ...
    "	#else",
    "		#if NCLAIMS==1",
    "			  fprintf(stderr, \"warning: only one claim defined, "
    "-N ignored\\n\");",
    "		#else",
    "			  fprintf(stderr, \"warning: no claims defined, -N "
    "ignored\\n\");",
    "		#endif",
    "			  if (!isdigit((int)argv[1][2]) && argc > 2 && "
    "argv[2][0] != '-')",
    "			  {	argc--; argv++;",
    "			  }",
    "	#endif",
    ...
    "		case 'P':",
    "			  if (!readtrail",
    "			  &&  isdigit((int) argv[1][2]))", /* was argv[1][2] ==
                                                              '_' */
    "			  {	int x = atoi(&argv[1][2]);",
    "				if (x != 0 && x != 1)",
    "				{	fprintf(efd, \"pan: bad option -P[01], "
    "ignored\\n\");",
    "				}",
    "				if (x == 0)",
    "				{	reversing &= ~1;",
    ...
    "				}",
    "				if (verbose)",
    "				fprintf(efd, \"pan: reversed *active* process "
    "creation %%s\\n\",",
    "					reversing&1?\"on\":\"off\");",
    "				break;",
    "			  } /* else */",
    ...
    "			  }",
    "	#else",
    "			  fprintf(efd, \"pan: option -P not recognized, "
    "ignored\\n\");",
    "	#endif",
    "#else",
    "			  fprintf(efd, \"pan: option -P not recognized, "
    ...
    "	#ifdef PERMUTED",
    "			  if (strncmp(&argv[1][1], \"p_normal\", "
    "strlen(\"p_normal\")) == 0)",
    "			  {	reversing &= ~2;",
    "				break;",
    "			  }",
    "			  reversing |=2;",
    "			  if (strncmp(&argv[1][1], \"p_permute\", "
    "strlen(\"p_permute\")) == 0)",
    "			  {	p_reorder = set_permuted;",
    "				break;",
    "			  }",
    "			  if (strncmp(&argv[1][1], \"p_rotate\", "
    "strlen(\"p_rotate\")) == 0)",
    ...
    "				break;",
    "			  }",
    "			  if (strncmp(&argv[1][1], \"p_randrot\", "
    "strlen(\"p_randrot\")) == 0)",
    "			  {	p_reorder = set_randrot;",
    "				break;",
    "			  }",
    "			  if (strncmp(&argv[1][1], \"p_reverse\", "
    "strlen(\"p_reverse\")) == 0)",
    ...
    "			  if (strncmp(&argv[1][1], \"p_permute\", "
    "strlen(\"p_permute\")) == 0",
    "			  ||  strncmp(&argv[1][1], \"p_rotate\",  "
    "strlen(\"p_rotate\")) == 0",
    "			  ||  strncmp(&argv[1][1], \"p_randrot\",  "
    "strlen(\"p_randrot\")) == 0",
    "			  ||  strncmp(&argv[1][1], \"p_reverse\", "
    "strlen(\"p_reverse\")) == 0)",
    "			  {	fprintf(efd, \"option %%s required compilation "
    "with -DPERMUTED\\n\",",
    "					argv[1]);",
    "				exit(1);",
    "			  }",
    ...
    "				srand(s_rand++);",
    "	#ifdef PERMUTED",
    "			  	do_hashgen = 1;", /* + randomize p_rotate,
                                                     p_reverse, p_permute */
    "			  	switch (rand()%%5) {",
    ...
    "					reversing |=2;",
    "					break;",
    "					/* choose once, then keep p_rotate "
    "fixed: */",
    ...
    "				break;",
    "	#else",
    "				fprintf(efd, \"option -rhash requires "
    "compilation with -DPERMUTED\\n\");",
    "				exit(1);",
    "	#endif",
    "			  }",
    ...
    "#else",
    "			  fprintf(efd, \"options -r is for models with "
    "embedded C code\\n\");",
    "			  break;",
    "#endif",
    "		case 'T':",
    "			  if (isdigit((int) argv[1][2]))", /* was argv[1][2] ==
                                                              '_' */
    "			  {	t_reverse = atoi(&argv[1][2]);",
    "				if (verbose)",
    "				printf(\"pan: reverse transition ordering "
    "%%s\\n\",",
    "					t_reverse?\"on\":\"off\");",
    "				break;",
    "			  }",
    ...
    "		/* -Un means that the nth worker should be instantiated as a "
    "proxy */",
    "		case 'U': proxy_pid = atoi(&argv[1][2]); break;",
    "		/* -W means this copy is started by a cluster-server as a "
    "remote */",
    ...
    "		#endif",
    "	}", /* it still works though, the later cores get states from the global
               q */
    "	#endif",
    "	#ifdef HAS_HIDDEN",
    ...
    "	if (hfns <= 0)",
    "	{	hfns = 1;",
    "		fprintf(efd, \"warning: using -k%%d as minimal usable "
    "value\\n\", hfns);",
    "	}",
    "#endif",
    "	omaxdepth = maxdepth;",
    ...
    "	fprintf(efd, \"warning: -i or -I work for safety properties "
    "only\\n\");",
    ...
    "\");",
    "			fprintf(stderr, \"but without -DNOREDUCE may be "
    "incomplete\\n\");",
    "		}",
    "	#endif",
    "#endif",
    ...
    "	#endif",
    "	#ifdef HAS_BADELSE",
    "		#error use of 'else' combined with i/o stmnts requires "
    "-DNOREDUCE",
    "	#endif",
    ...
    "		#ifdef BFS",
    "		fprintf(efd, \"\t(this type of rv is also not compatible with "
    "-DBFS)\\n\");",
    "		#endif",
    "	#endif",
    "#endif",
    ...
    "	if (!noends)",
    "	fprintf(efd, \"warning: use of rendezvous with BFS does not preserve "
    "all invalid endstates\\n\");",
    "#endif",
    ...
    "			  fprintf(efd, \"requires -l flag to fully "
    "verify\\n\");",
    "#else",
    ...
    "#endif",
    "	fprintf(fd, \"\t-hN use different hash-seed N:0..499 (defaults to "
    "-h0)\\n\");",
    "	fprintf(fd, \"\t-hash generate a random hash-polynomial for -h0 (see "
    "also -rhash)\\n\");",
    ...
    "#ifdef PERMUTED",
    "	fprintf(fd, \"\t-p_permute randomize order in which processes are "
    "scheduled (see also -rhash)\\n\");",
    "	fprintf(fd, \"\t-p_reverse reverse order in which processes are "
    "scheduled (see also -rhash)\\n\");",
    ...
    "#ifdef HAS_CODE",
    "	fprintf(fd, \"\t-r  read and execute trail - can add "
    "-v,-n,-PN,-g,-C\\n\");",
    "	fprintf(fd, \"\t-r trailfilename  read and execute trail in "
    "file\\n\");",
    "	fprintf(fd, \"\t-rN read and execute N-th error trail\\n\");",
    "	fprintf(fd, \"\t-C  read and execute trail - columnated output (can "
    "add -v,-n)\\n\");",
    "	fprintf(fd, \"\t-r -PN read and execute trail - restrict trail output "
    "to proc N\\n\");",
    ...
    "	fprintf(fd, \"\t-rhash use random hash-polynomial and randomly choose "
    "-p_rotateN, -p_permute, or p_reverse\\n\");",
    "#ifdef BITSTATE",
    ...
    "	fprintf(fd, \"\t-t_reverse  reverse order in which transitions are "
    "explored\\n\");",
    ...
    "{	if (!svtack->next)",
    "	{  svtack->next = (Svtack *) emalloc(sizeof(Svtack));",
    "	   svtack->next->body = emalloc(vsize*sizeof(char));",
    "	   svtack->next->lst = svtack;",
    "	   svtack->next->m_delta = vsize;",
    "	   svmax++;",
    "	} else if (vsize > svtack->next->m_delta)",
    "	{  svtack->next->body = emalloc(vsize*sizeof(char));",
    "	   svtack->next->lst = svtack;",
    "	   svtack->next->m_delta = vsize;",
    "	   svmax++;",
    ...
    "		freebodies = freebodies->next;",
    "		processes[h]->next = (TRIX_v6 *) 0;",
    ...
    "	z = empty_chunks[nr];",
    "	empty_chunks[nr] = empty_chunks[nr]->next;",
    "	z->ptr = p;",
    "	z->next = filled_chunks[nr];",
    "	filled_chunks[nr] = z;",
    "}",
    "int",
    ...
    "	if (sav)",
    "	{	if (!stack->next)",
    "		{	stack->next = (_Stack *) emalloc(sizeof(_Stack));",
    "			stack->next->lst = stack;",
    "			smax++;",
    "		}",
    "		stack = stack->next;",
    "	#ifndef XUSAFE",
    ...
    "	{	if (!stack->next)",
    "		{	stack->next = (_Stack *) emalloc(sizeof(_Stack));",
    "			stack->next->body = emalloc(Maxbody * sizeof(char));",
    "			stack->next->lst = stack;",
    "			smax++;",
    "		}",
    "		stack = stack->next;",
    ...
    "	#if !defined(NOCOMP) && !defined(HC)",
    "		#ifdef BFS_PAR",
    "			bfs_prepmask(3); /* delproc - no chance in proc_offset "
    "or proc_skip */",
    "		#endif",
    ...
    "	#if !defined(NOCOMP) && !defined(HC)",
    "		#ifdef BFS_PAR",
    "			bfs_prepmask(3); /* delq - no change in q_offset or "
    "q_skip */",
    "		#endif",
    ...
    "{	H_el *v, *last = 0;",
    "	if (H_tab == S_Tab)",
    "	{	for (v = Free_list; v && ((int) v->tagged >= n); v=v->next)",
    "		{	if ((int) v->tagged == n)",
    "			{	if (last)",
    "					last->next = v->next;",
    "				else",
    "gotcha:				Free_list = v->next;",
    "				v->tagged = 0;",
    "				v->next = 0;",
    "	#ifdef COLLAPSE",
    "				v->ln = 0;",
    ...
    "#endif",
    "	H_tab = tmp;",
    "	for (v = S_Tab[j1_spin]; v; Zh++, last=v, v=v->next)",
    ...
    "		last->next = v->next;",
    "	else",
    "		S_Tab[j1_spin] = v->next;",
    "	v->tagged = (unsigned) n;",
    "#if !defined(NOREDUCE) && !defined(SAFETY)",
    "	v->proviso = 0;",
    "#endif",
    "	v->next = last = (H_el *) 0;",
    "	for (w = Free_list; w; Fa++, last=w, w = w->next)",
    "	{	if ((int) w->tagged <= n)",
    "		{	if (last)",
    "			{	v->next = w;",
    "				last->next = v;",
    ...
    "	#endif",
    "		H_tab = tmp2;",
    "		for (tmp = S_Tab[j1_spin]; tmp; Zn++, tmp = tmp->next)",
    "		{	m = memcmp(((char *)&(tmp->state)),v,n);",
    "			if (m <= 0)",
    ...
    "		#if defined(BFS_PAR) && !defined(BFS_SEP_HASH)",
    "			SS = (uchar *) "
    "sh_pre_malloc((ulong)(ONE_L<<(ssize-3)));",
    "		#else",
    ...
    "	#endif",
    "	#if !defined(MA) || defined(COLLAPSE)",
    "		#if NCORE>1 || (defined(BFS_PAR) && defined(USE_TDH) && "
    "!defined(WIN32) && !defined(WIN64))",
    "			if (!readtrail)",
    "			{	void init_HT(ulong);",
    "				init_HT((ulong) (ONE_L<<ssize)*sizeof(H_el "
    "*));",
    "			#if defined(TRIX) || (defined(BFS_PAR) && "
    "defined(COLLAPSE))",
    "				set_H_tab(); /* need both */",
    "			#endif",
    "			} else",
    ...
    "	/* to make this mode work, also replace emalloc with grab_shared "
    "inside store MA routines */",
    "#endif",
    "",
    "	if (!dfa_store(Info))",
    "	{	if (pbit == 0",
    ...
    "			   for (;; hcmp++, olst = tmp, tmp = tmp->next)",
    "			   { 	m = memcmp(((char *)&(tmp->state)), v, n);",
    "				if (m == 0)	/* match */",
    "				{	break;",
    "				} else if (m < 0) /* insert */",
    ...
    "					tmp = ntmp;",
    "					break;",
    "				} else if (!tmp->next)	/* append */",
    "				{	tmp->next = grab_state(n);",
    "					tmp = tmp->next;",
    "					break;",
    "			}  }	}",
    "			if (m != 0)",
  ...
  (p.m. unsure if this preserves the guarantee of finding the
   shortest counter-example - so not enabled by default)
    ...
    "	#else",
    "		ohash_sd   = (volatile char *) sh_pre_malloc(x * (ulong) "
    "T_VSZ);",
    "	#endif",
    ...
    "#else",
    "	ohash_hv = (volatile uint32_t *) sh_pre_malloc(x * (ulong) "
    "sizeof(uint32_t));",
    "#endif",
    ...
    "	#else",
    "	ohash_inq  = (volatile uchar *) sh_pre_malloc(x * (ulong) "
    "sizeof(uchar));",
    "	#endif",
    "#endif",
    "}",
    ...
    "		if (s == (uchar *) Lstate)",
    "		{	depthfound = depth - (BASE+1)*(L_bound - now._l_bnd - "
    "1);",
    ...
    "		hash_v = K1;",
    "	} while (hash_v == T_FREE || hash_v == T_STAT);	/* unlikely, hash_v 0 "
    "or 1 */",
    "",
    "	T_BUSY = ((uint32_t) hash_v & ~((uint32_t) T_STAT));	/* hash with "
    "status bit 0 */",
    "	T_DONE = ((uint32_t) hash_v |  ((uint32_t) T_STAT));	/* hash with "
    "status bit 1 */",
    "#ifdef HC",
    ...
    "			if (*t_entry == T_FREE && cas(t_entry, T_FREE, "
    "T_BUSY))",
    "			{",
    "#ifndef HC",
    "				memcpy((char *) &ohash_sd[ix * vs], vin, nin);",
    "#else",
    "				memcpy((char *) &ohash_sd[ix * vs], (char *) "
    "ohash_hc, vs);",
    "#endif",
    "#if defined(USE_TDH) && defined(Q_PROVISO)",
    ...
    "			{	usleep(2);	/* wait */",
    "			}",
    "			if (*t_entry == T_DONE /* (first) hash matches, check "
    "data */",
    "#ifndef HC",
    "			&&   memcmp((char *) &ohash_sd[ix * vs], vin, nin) == "
    "0)",
    "#else",
    "			&&   memcmp((char *) &ohash_sd[ix * vs], (char *) "
    "ohash_hc, vs) == 0)",
    "#endif",
    "			{",
    "#if defined(USE_TDH) && defined(Q_PROVISO)",
