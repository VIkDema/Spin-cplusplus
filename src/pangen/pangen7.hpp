...
static const char *pan_par[] = {
    ...
    "};",
    "",
    "struct BFS_shared {	/* about 13K for BFS_MAXPROCS=16 and "
    "BFS_MAXLOCKS=1028 */",
    ...
    "	volatile uchar	bfs_flag[BFS_MAXPROCS]; /* running 0, normal exit 1, "
    "abnormal 2 */",
    "	volatile uchar	bfs_idle[BFS_MAXPROCS]; /* set when all input queues "
    "are empty */",
    "#ifdef BFS_DISK",
    "	volatile uchar	bfs_out_cnt[BFS_MAXPROCS]; /* set when core writes a "
    "state */",
    "#endif",
    "",
    "#ifdef BFS_QSZ",
    ...
    "#endif",
    "static long	bfs_sent;	/* nr msgs sent -- local to each "
    "process */",
    ...
    "};",
    "",
    "static ulong bfs_count[DELETED+1];	/* indexed with bfs_types: EMPTY=0, "
    "STATE=1, DELETED=2 */",
    "",
    ...
    "{",
    "	if (cycles)",
    "	{	fprintf(stderr, \"pan: cycle detection is not supported in "
    "this mode\\n\");",
    "		exit(1);",
    "	}",
    "",
    ...
    "",
    "	shared_memory            = (BFS_shared *) bfs_get_shared_mem(key, "
    "n);	/* root */",
    "	shared_memory->allocator = (uchar *) shared_memory + "
    "sizeof(BFS_shared);",
    ...
    "",
    "	n = shared_memory->mem_left / (Cores + Cores/(BFS_RESERVE)); /* keep "
    "some reserve */",
    "",
    "	if ((n%%sizeof(void *)) != 0)",
    ...
    "	x_critical(BFS_GLOB);",
    "",
    "	while (shared_memory->started != Cores)	/* wait for all cores to "
    "connect */",
    "	{	usleep(1);",
    "	}",
    "",
    ...
    "#endif",
    "			/* last resort: start dropping states when out of "
    "memory */",
    ...
    "				{	warned_loss++;",
    "					bfs_printf(\"out of shared memory - "
    "losing states\\n\");",
    "				}",
    "				bfs_punt++;",
    "			}",
    ...
    "				}",
    "			} else",
    "			{	/* wait for quit or idle bit to be reset by "
    "root */",
    ...
    "#endif",
    "	#ifdef BFS_CHECK",
    "				bfs_printf(\"toggle: recv from %%d, send to "
    "%%d\\n\",",
    "					bfs_toggle, 1 - bfs_toggle);",
    "	#endif",
    "			}",
    ...
    "		((double) bfs_total_shared - (double) "
    "bfs_left)/(1024.*1024.));",
    "}",
    "",
    "void",
    ...
    "	#endif",
    "	if (verbose)",
    "		bfs_printf(\"states sent %%7ld recvd %%7ld stored %%8g sleeps: "
    "%%4ld, %%4ld, %%ld\\n\",",
    "			bfs_sent, bfs_rcvd, nstates, bfs_wcount, bfs_gcount, "
    "bfs_sleep_cnt);",
    ...
    "				bfs_lname[BFS_STATE], count/(BFS_MAXLOCKS - "
    "BFS_STATE));",
    "#endif",
    "		  }",
    "		  fflush(stdout);",
    ...
    "	if (bfs_free_hold)",
    "	{	x = bfs_free_hold;",
    "		bfs_free_hold = bfs_free_hold->next;",
    "	} else",
    ...
    "		{	bfs_t->lstate->tagged = 0;	/* bfs_par state "
    "removed from q */",
    "		}",
    "	#endif",
    "#endif",
    ...
    "",
    "void",
    "bfs_explore_state(BFS_Slot *v)		/* generate all successors of "
    "v */",
    "{	BFS_Trail *otrpt;",
    "	Trans *t;",
    "#ifdef HAS_UNLESS",
    ...
    "	#ifdef VERBOSE",
    "			bfs_printf(\"%%3ld: proc %%d PreSelected "
    "(tau=%%d)\\n\", ",
    "				depth, II, trpt->tau);",
    "	#endif",
    "			goto Repeat_two;",
    ...
    "		E_state = 0;",
    "#endif",
    "		for (t = trans[ot][tt]; t; t = t->next)	/* all process "
    "transitions */",
    "		{",
    "#ifdef BFS_CHECK",
    "			assert(t_id_lkup[t->t_id] == t); /* for reverse lookup "
    "in bfs_unpack_state */",
    "#endif",
    "#ifdef VERBOSE",
    ...
    "			{ fprintf(efd, \"error:	rendezvous stmnt in the escape "
    "clause\\n\");",
    "			  fprintf(efd, \"	of unless stmnt not compatible "
    "with -DBFS\\n\");",
    "			  pan_exit(1);",
    "			}",
    "	#endif",
    ...
    "			{	((P0 *)_this)->_p = t->st;",
    "			}",
    "			/* use the ostate ptr, with type *H_el, to temporarily "
    "store *BFS_Trail */",
    "#ifdef BFS_NOTRAIL",
    "			ntrpt->ostate = (H_el *) 0;	/* no error-traces in "
    "this mode */",
    "#else",
    ...
    "move */",
    "			} else",
    "			{	ntrpt->tau = 0;		/* no timeout or "
    "preselect etc */",
    "			}",
    "#ifdef VERI",
    "			ntrpt->tau |= trpt->tau&4;	/* if claim, inherit "
    "*/",
    "			if (boq == -1 && !(ntrpt->tau&8))	/* unless rv "
    "or atomic */",
    ...
    "			printf(\"reverses %%d, %%d to %%d,\", t->forw, tt, "
    "t->st);",
    "			printf(\" %%s [abit=%%d,adepth=%%d,\", t->tp, "
    "now._a_t, 0);",
    "			printf(\"tau=%%d,%%d]\\n\", trpt->tau, (trpt-1)->tau);",
    "			x_critical(BFS_PRINT);",
    "#endif",
    ...
    "	#ifdef VERBOSE",
    "		bfs_printf(\"%%3ld: proc %%d UnSelected (_n=%%d, "
    "tau=%%d)\\n\", ",
    ...
    "		{	BFS_Trail *x = (BFS_Trail *) trpt->ostate; /* pre-rv "
    "state */",
    "			if (x && ((x->tau&8) || (x->tau&32))) /* break atomic "
    "or preselect */",
    ...
    "				printf(\"repush state\\n\");",
    "#endif",
    "				bfs_push_state(NULL, x, x->o_tt); /* repush rv "
    "fail */",
    ...
    "depth, II);",
    "	#endif",
    "			bfs_store_state(trpt, boq);", /* doesn't store it but
                                                         queues it */
    "	#ifdef VERBOSE",
    "			printf(\"%%3ld: proc -1 reverses -1, (stutter "
    "move)\\n\", depth, II);",
    "	#endif",
    ...
    "#ifdef BFS_FIFO",
    "int",
    "bfs_empty(int p)", /* return Cores if all empty or index of first non-empty
                           q of p */
    "{	int i;",
    "	const int dst = 0;",
    "	for (i = 0; i < Cores; i++)",
    ...
    "#endif",
    "	bfs_ewrite(bfs_out_fd[who_are_you], (const void *) n->s_data->t_info, "
    "sizeof(BFS_Trail));",
    "	bfs_ewrite(bfs_out_fd[who_are_you], (const void *) &vsize, "
    "sizeof(ulong));",
    ...
    "",
    "#if NRUNS>0",
    "	bfs_ewrite(bfs_out_fd[who_are_you], (const void *) "
    "&(n->s_data->omask), sizeof(EV_Hold *));",
    "#endif",
    "#ifdef Q_PROVISO",
    "	bfs_ewrite(bfs_out_fd[who_are_you], (const void *) "
    "&(n->s_data->lstate), sizeof(H_el *));",
    "#endif",
    "#if SYNC>0",
    "	bfs_ewrite(bfs_out_fd[who_are_you], (const void *) &(n->s_data->boq), "
    "sizeof(short));",
    "#endif",
    "#if VERBOSE",
    "	bfs_ewrite(bfs_out_fd[who_are_you], (const void *) &(n->s_data->nr), "
    "sizeof(ulong));",
    "#endif",
    ...
    "	BFS_Slot *n;",
    "	who_are_you = (rand()%%Cores);", /* important: all to the same cpu, but
                                            reversed */
    ...
    " #endif",
    "		n->next = (BFS_Slot *) "
    "shared_memory->head[dst][who_are_you][who_am_i];",
    ...
    "		(BFS_Slot *) "
    "&(shared_memory->bfsq[dst][who_are_you][who_am_i][z])));",
    "#else",
    ...
    "		shared_memory->head[dst][who_are_you][who_am_i] = n;",
    "	  } else",
    "	  {	shared_memory->tail[dst][who_are_you][who_am_i]->next = n;",
    ...
    "#endif", /* BFS_QSZ */
    "#ifdef VERBOSE",
    "	bfs_printf(\"PUT STATE (depth %%ld, nr %%u) to %%d -- s_data: "
    "%%p\\n\",",
    ...
    "		for (n = shared_memory->head[src][who_am_i][bfs_qscan]; n; n = "
    "n->next)",
    "		{	if (n->type != DELETED)",
    "			{	break;",
    "		}	}",
    ...
    "	#endif",
    "		if (n->next)",
    "		{	shared_memory->head[src][who_am_i][bfs_qscan] = "
    "n->next;",
    ...
    "		shared_memory->head[src][who_am_i][bfs_qscan] = n->next;",
    "		#if defined(BFS_FIFO) && defined(BFS_CHECK)",
    "			assert(n->type == STATE);",
    "		#endif",
    "		n->next = (BFS_Slot *) 0;",
    "	#endif",
    "	#ifdef BFS_DISK",
    "		/* the states actually show up in reverse order (FIFO iso "
    "LIFO) here */",
    "		/* but that doesn't really matter as long as the count is "
    "right */",
    ...
    "		n = n->next)",
    "	{	if (n->type == DELETED && n->next)",
    "		{	shared_memory->dels[0][who_am_i][i] = n->next;",
    "			n->next = (BFS_Slot *) 0;",
    "			return n;",
    "	}	}",
    ...
    "void",
    "bfs_release_trail(BFS_Trail *t)", /* call in bfs_sink_disk (not
                                          bfs_recycle) */
    "{	BFS_T_Hold *h;",
    " #ifdef BFS_CHECK",
    "	assert(t);",
    ...
    "	{	t = bfs_free_slot;",
    "		bfs_free_slot = bfs_free_slot->next;",
    "		t->next = (BFS_Slot *) 0;",
    "	} else",
    ...
    "	} else",
    "	{	t->s_data = (BFS_State *) sh_malloc((ulong) "
    "sizeof(BFS_State));",
    "	}",
    "",
    ...
    "		h->sz = n;",
    "#endif",
    "		h->sv = (State *) sh_malloc((ulong)(sizeof(State) - VECTORSZ + "
    "n));",
    "	}",
    ...
    "{	EV_Hold *h;",
    "",
    "	for (h = kept[v->sz]; h; h = h->next) /* check local list */",
    "	{	if (v->nrpr == h->nrpr",
    "		&&  v->nrqs == h->nrqs",
    "#if !defined(NOCOMP) && !defined(HC)",
    "		&&  (v->sv == h->sv || memcmp((char *) v->sv, (char *) h->sv, "
    "v->sz) == 0)",
    "#endif",
    "#ifdef TRIX",
    "		)",
    ...
    "	#if NRUNS>0",
    "		#if VECTORSZ>32000",
    "		&&  (memcmp((char *) v->po, (char *) h->po, v->nrpr * "
    "sizeof(int)) == 0)",
    "		&&  (memcmp((char *) v->qo, (char *) h->qo, v->nrqs * "
    "sizeof(int)) == 0)",
    "		#else",
    "		&&  (memcmp((char *) v->po, (char *) h->po, v->nrpr * "
    "sizeof(short)) == 0)",
    ...
    "sizeof(short)) == 0)",
    "		#endif",
    "		&&  (memcmp((char *) v->ps, (char *) h->ps, v->nrpr * "
    "sizeof(uchar)) == 0)",
    "		&&  (memcmp((char *) v->qs, (char *) h->qs, v->nrqs * "
    "sizeof(uchar)) == 0))",
    "	#else",
    "		)",
    "	#endif",
    ...
    "	{	v->next = kept[v->sz];	/* keep the original owner field */",
    "		kept[v->sz] = v;",
    "		/* all ptrs inside are to shared memory, but next is local */",
    "	}",
    "}",
    "",
    "EV_Hold *",
    ...
    "{	EV_Hold *h;",
    "",
    "	for (h = kept[n]; h; h = h->next)",
    "	{	if ((now._nr_pr == h->nrpr)",
    "		&&  (now._nr_qs == h->nrqs)",
    "#if !defined(NOCOMP) && !defined(HC) && NRUNS>0",
    "		&& ((char *) Mask == h->sv || (memcmp((char *) Mask, h->sv, n) "
    "== 0))",
    "#endif",
    "#ifdef TRIX",
    "		)",
    ...
    "		h->nrqs = now._nr_qs;",
    "#if !defined(NOCOMP) && !defined(HC) && NRUNS>0",
    "		h->sv = (char *) Mask;	/* in shared memory, and never "
    "modified */",
    "#endif",
    ...
    "	sv_populate();",
    "#endif",
    "#if defined(VERI) && defined(Q_PROVISO) && !defined(BITSTATE) && "
    "defined(FULLSTACK)",
    ...
    "		} else",
    "	#endif",
    "		if (!bfs_do_store((char *)&now, vsize))	/* includes "
    "bfs_visited */",
    "		{	nstates++;			/* local count */",
    "			trpt->tau |= 64;		/* bfs: succ outside "
    "stack */",
    "	#if SYNC",
    "			if (boq == -1 && oboq != -1)	/* post-rv */",
    "			{	BFS_Trail *x = ",
    ...
    "	{	w = shared_memory->sh_owner[which]; /* sh_locks[which] could "
    "be 0 by now */",
    "		assert(w >= 0 && w <= BFS_MAXPROCS);",
    "		if (w > 0 && shared_memory->bfs_flag[w-1] == 2)",
    "		{	/* multiple processes can get here; only one should do "
    "this: */",
    "			if (tas(&(shared_memory->bfs_data[w - 1].override)) == "
    "0)",
    "			{	fprintf(stderr, \"cpu%%02d: override lock %%d "
    "- held by %%d\\n\",",
    "				who_am_i, which, "
    "shared_memory->sh_owner[which]);",
    "#ifdef BFS_CHECK",
    ...
    "	{	fprintf(stderr, \"cpu%%02d: cannot happen lock %%d count "
    "%%d\\n\", who_am_i,",
    ...
    "	for (p = 0; p <= _NP_; p++)",
    "	{	memcpy((void *) q, (const void *) reached[p], (size_t) "
    "NrStates[p]);",
    "		q += NrStates[p];",
    "	}",
    "}",
    ...
    "	s = &shared_memory->bfs_data[who_am_i];",
    "	if (who_am_i == 0)",
    "	{	shared_memory->bfs_flag[who_am_i] = 3; /* or else others don't "
    "stop */",
    "		bfs_gcount = 0;",
    "		for (i = 1; i < Cores; i++) /* start at 1 not 0 */",
    "		{	while (shared_memory->bfs_flag[i] == 0)",
    "			{	sleep(1);",
    "				if (bfs_gcount++ > WAIT_MAX)	/* excessively "
    "long wait */",
    "				{	printf(\"cpu00: give up waiting for "
    "cpu%%2d (%%d cores)\\n\", i, Cores);",
    "					bfs_gcount = 0;",
    "					break;",
    "			}	}",
    ...
    "		if (shared_memory->mem_left < n + 7)", /* 7 for possible
                                                          alignment */
    "		{	x_critical(BFS_MEM);",
    ...
    "	assert(who_am_i == 0);",
    "",
    "	shared_mem_id = shmget(key, n, 0600 | IPC_CREAT | IPC_EXCL);	/* "
    "create */",
    "	if (shared_mem_id == -1)",
    "	{	fprintf(stderr, \"cpu%%02d: tried to get %%d MB of shared "
    "memory\\n\",",
    "			who_am_i, (int) n/(1024*1024));",
    "		perror(\"shmget\");",
    "		exit(1);",
    "	}",
    "	if ((rval = (char *) shmat(shared_mem_id, (void *) 0, 0)) == (char *) "
    "-1) /* attach */",
    "	{	perror(\"shmat\");",
    "		exit(1);",
    "	}",
    ...
    "detach */",
    "		if (who_am_i == 0)",
    "		{	shmctl(shared_mem_id, IPC_RMID, (void *) 0);	/* "
    "remove */",
    "	}	}",
    "	if (who_am_i == 0)",
    "	{	printf(\"done\\n\");",
    ...
    "		}",
    "#endif",
    "		temp_id = shmget(key, n, 0600);		/* check for stale "
    "copy */",
    "		if (temp_id != -1)",
    ...
    "		}	}",
    "",
    "		temp_id = shmget(key, n, 0600 | IPC_CREAT | IPC_EXCL);	/* "
    "create new */",
    "		if (temp_id == -1)",
    "		{	n -= delta;",
    "			break;",
    ...
    "	  { fprintf(stderr, \"\\t32MB is the default; increase it to 1 GB "
    "with:\\n\");",
    ...
    "	  { fprintf(stderr, \"\\t32MB is the default; increase it to 30 GB "
    "with:\\n\");",
    "	    fprintf(stderr, \"\\tsudo /sbin/sysctl "
    "kernel.shmmax=32212254720\\n\");",
    "	    fprintf(stderr, \"\\tsudo /sbin/sysctl "
    "kernel.shmall=7864320\\n\");",
    "	    fprintf(stderr, \"\\tor for 60 GB:\\n\");",
    "	    fprintf(stderr, \"\\tsudo /sbin/sysctl "
    "kernel.shmmax=60000000000\\n\");",
    "	    fprintf(stderr, \"\\tsudo /sbin/sysctl "
    "kernel.shmall=30000000\\n\");",
    "	  } else",
    ...
    "	for (i = 0; i < Cores; i++)",
    "	{	sprintf(fnm, \".spin/q%%d_%%d_%%d\", 1-bfs_toggle, i, "
    "who_am_i);",
