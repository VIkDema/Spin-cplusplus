...
static void put_sub(Element *e, int Tt0, int Tt1) {
  ...
  if (e->n->ntyp == D_STEP) {
    ...
    fprintf(fd_tm, "// STATE %d - %s:%d - [", e->seqno, e->n->fn->name,
            e->n->ln);
    comment(fd_tm, e->n, 0);
    fprintf(fd_tm, "]\n\t\t");
...
static void doforward(FILE *tm_fd, Element *e) {
  ...
  if (deadvar && !lexer_.GetHasCode())
    for (u = e->dead; u; u = u->nxt) {
      ...
      fprintf(tm_fd, "\t\t/* dead %d: %s */  ", u->special, u->var->name);
...
void putstmnt(FILE *fd, Lextok *now, int m) {
  ...
  case 'r':
    ...
    /* set */ for (v = now->rgt, i = 0; v; v = v->rgt, i++) {
      ...
      if (v->lft->ntyp != CONST && v->lft->ntyp != EVAL &&
          v->lft->sym != NULL && strcmp(v->lft->sym->name, "_") != 0) {
        nocast = 1;
        _isok++;
        putstmnt(fd, v->lft, m);
        _isok--;
      ...
      if (v->lft->ntyp != CONST && v->lft->ntyp != EVAL &&
          v->lft->sym != NULL && strcmp(v->lft->sym->name, "_") != 0 &&
          (v->lft->ntyp != NAME || v->lft->sym->type != CHAN)) {
        fprintf(fd, ";\n#ifdef VAR_RANGES");
        fprintf(fd, "\n\t\tlogval(\"");
