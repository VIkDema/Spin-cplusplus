...
static int sametrans(Transition *s, Transition *t) {
  if (strcmp(s->name->name, t->name->name) != 0)
    return 0;
  return isequal(s->cond, t->cond);
}

static Node *Prune(Node *p) {
  if (p)
    switch (p->ntyp) {
    case PREDICATE:
    case NOT:
...
static Node *combination(Node *s, Node *t) {
  Node *nc;
#ifdef NXT
  Node *a = nonxt(s);
  Node *b = nonxt(t);
...
static int all_trans_match(State *a, State *b) {
  Transition *s, *t;
  int found, result = 0;
...
#ifdef BUCKY
static int all_bucky(State *a, State *b) {
  Transition *s, *t;
  int found, result = 0;
...
static void printstate(State *b) {
...
void addtrans(Graph *col, char *from, Node *op, char *to) {
  State *b;
  Transition *t;
