...
void append_to_dumpbuf(char *s) {
  size_t len = strlen(s);
  size_t size_needed = dumpbuf_size + len + 1;
  if (size_needed > dumpbuf_capacity) {
    dumpbuf = static_cast<char *>(
        tl_erealloc(dumpbuf, size_needed, dumpbuf_capacity));
...
void ini_trans(void) {
  ...
  dumpbuf = static_cast<char *>(tl_emalloc(dumpbuf_capacity));
  dumpbuf_size = 0;
...
static void push_stack(Graph *g) {
  if (!g)
    return;
...
static int has_clause(int tok, Graph *p, Node *n) {
  Node *q, *qq;
...
static Graph *findgraph(char *nm) {
  Graph *p;
  Mapping *m;
...
static int choueka(Graph *p, int count) {
  int j, k, incr_cnt = 0;
...
static int set_prefix(char *pref, int count, Graph *r2) {
...
static void fsm_trans(Graph *p, int count, char *curnm) {
  ...
  for (s = p->outgoing; s; s = s->next) {
    ...
    if (tl_verbose) {
      printf("maxred=%d, count=%d, curnm=%s, nwnm=%s ", Max_Red, count, curnm,
             nwnm);
