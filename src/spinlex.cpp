...
static void set_cur_scope(void) {
  int i;
  char tmpbuf[256];
...
static int specials[] = {'}',    ')',    ']',  OD,    FI,   ELSE, BREAK, C_CODE,
                         C_EXPR, C_DECL, NAME, CONST, INCR, DECR, 0};

int follows_token(int c) {
  int i;
...
void zap_deferred(void) { (void)unlink(TMP_FILE2); }

int put_deferred(void) {
  int c, cnt;
  if (!defer_fd) {
...
static int scan_to(int stop, int (*tst)(int), char *buf, int bufsz) {
  int c, i = 0;
...
again:
  ...
  case '\n': /* newline */
    ...
    if (implied_semis
        /*	&&  context	*/
        && in_seq && par_cnt == 0 && follows_token(last_token)) {
      if (last_token == '}') {
        do {
          c = Getchar();
          if (c == '\n') {
    ...
    if (!in_comment) {
      ...
      if (c == SELECT && Inlining < 0) {
        ...
        if (b - a <= 32) {
          ...
          char buf[256];
          sprintf(buf, "%s = %d; do ", name, a);
          push_back(buf);
          sprintf(buf, ":: (%s < %d) -> %s++ ", name, b, name);
          push_back(buf);
          push_back(":: break od; ");
        }
        goto again;
...
int yylex(void) {
  ...
  if (hold) {
    ...
    c = lex();
    if (last == ELSE && c != SEMI && c != ARROW && c != FI) {
      hold = c;
      last = 0;
      last_token = SEMI;
      return SEMI;
    }
    if (last == '}' && c != PROCTYPE && c != INIT && c != CLAIM && c != SEP &&
        c != FI && c != OD && c != '}' && c != UNLESS && c != SEMI &&
        c != ARROW && c != EOF) {
      hold = c;
      last = 0;
      last_token = SEMI;
    ...
    if (c == SEMI || c == ARROW) { /* if context, we're not in a typedef
